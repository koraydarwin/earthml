import numpy as np
 ...
 NOISE_TYPE2DEFAULT_PARAMS = {
     ...
     "additive_exponential"        : ADDITIVE_NOISE_PARAMS,
     "multiplicative_exponential"  : MULTIPLICATIVE_NOISE_PARAMS,
     "multiplicative_rayleigh"     : MULTIPLICATIVE_NOISE_PARAMS}
 ...
 def noise_aug(data,
     ...
     snr: float or int
        ...
        standard deviation of noise (signal to noise ratio).
     ...
     snr_thres: float or int
        signal to noise ratio threshold to be augmented.
 ...
 def exp_add_noise(data, scale_params):
     ...
     for i in range(num_channels):
         channel_scale = np.random.uniform(
           scale_params["low_bound_scale"],
           scale_params["up_bound_scale"])
        noise = signs * np.random.exponential(channel_scale * max(data[:,i]), num_samples)
